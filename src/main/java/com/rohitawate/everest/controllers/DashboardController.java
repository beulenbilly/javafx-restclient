...
public class DashboardController implements Initializable {
	...
	public void initialize(URL url, ResourceBundle rb) {
		...
		copyBodyButton.setOnAction(e -> {
			...
			snackbar.enqueue(createSnackbarEvent("Response body copied to clipboard.", 5000));
		});
	...
	void sendRequest() {
		...
		try {
			...
			if (address.equals("")) {
				showLayer(ResponseLayer.PROMPT);
				snackbar.enqueue(createSnackbarEvent("Please enter an address.", 3000));
				return;
			}
			...
			showLayer(ResponseLayer.PROMPT);
			snackbar.enqueue(createSnackbarEvent("Invalid address. Please verify and try again.", 3000));
		} catch (Exception E) {
	...
	private void onFailed(Event event) {
		...
		if (throwable.getClass() == NullResponseException.class) {
			...
			snackbar.enqueue(createSnackbarEvent("Resource moved permanently. Redirecting...", 3000));
			requestManager = null;
			sendRequest();
			return;
		}
	...
	private void prettifyResponseBody(String body, String contentType) {
		...
		try {
				...
				switch (contentType.toLowerCase()) {
					...
					case "text/html":
						...
						if (Desktop.getDesktop().isSupported(Desktop.Action.BROWSE)) {
							snackbar.enqueue(createSnackbarEvent("Open link in browser?", "YES", 5000, e -> {
								snackbar.close();
								new Thread(() -> {
									try {
			...
			snackbar.enqueue(createSnackbarEvent(errorMessage, 5000));
	...
	private JFXSnackbar.SnackbarEvent createSnackbarEvent(String message, long timeoutMillis) {
		return createSnackbarEvent(message, null, timeoutMillis, null);
	}

	private JFXSnackbar.SnackbarEvent createSnackbarEvent(String message, String actionText, long timeoutMillis, EventHandler<? super MouseEvent> actionHandler) {
		...
		if (actionText != null && !actionText.isEmpty()) {
			JFXButton action = new JFXButton(actionText);
			...
			StackPane actionContainer = new StackPane(action);
		...
		return new JFXSnackbar.SnackbarEvent(bPane, Duration.millis(timeoutMillis));
